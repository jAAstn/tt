...
//
// For 'currentWindow' we filter on `win.focused` at the START of each
// iteration. Filtering AFTER capturing tabs would let the loop silently
// fall through to non-focused windows whenever the focused window has only
// internal/blocked tabs (e.g. when the user triggers the save from the
// options page itself, which contains only `chrome-extension://...` URLs).
async function captureSavedSession(scope) {
  const allWindows = await chrome.windows.getAll({ populate: true });
  const sessionWindows = [];
  for (const win of allWindows) {
    // When scoped to the current window, ignore every window that is not
    // the focused one -- BEFORE doing any capture work -- so that an empty
    // filtered focused window never lets the loop drift into other windows.
    if (scope === 'currentWindow' && !win.focused) continue;
    const savedTabs = [];
    ...
    // Safe to break unconditionally now: when scope is 'currentWindow' we
    // already skipped every non-focused window above, and 'allWindows'
    // intentionally keeps iterating.
    if (scope === 'currentWindow') break;
  }
