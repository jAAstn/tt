...
async function queueTabForSuspensionAsPromise(tab, forceLevel) {
    ...
    // Wenn die Queue nach einem Service Worker Wake-up undefined ist, neu initialisieren
    if (!_suspensionQueue) {
      await initAsPromised();
    }
...
function unqueueTabForSuspension(tab) {
    // Check hinzufügen, ob die Queue existiert
    if (!_suspensionQueue) {
      gsUtils.warning(tab ? tab.id : null, QUEUE_ID, 'unqueueTabForSuspension: suspensionQueue not initialized');
      return; 
    }
