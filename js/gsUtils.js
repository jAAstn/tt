...
async discardTabSafely(tabId) {
    if (!tabId) return;
    try {
        // Der Chrome API-Aufruf, der den Absturz verursacht hat, wird jetzt geschützt.
        await chrome.tabs.discard(tabId);
        gsUtils.log(tabId, 'Tab successfully discarded.');
    } catch (e) {
        // Fehler abfangen (z.B. "No such tab with id: X" oder "Cannot discard a tab")
        // und den Service Worker vor dem Absturz bewahren.
