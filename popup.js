...
(async () => {
  ...
  // Überarbeitete Whitelist-Prüfung inklusive RegEx-Erkennung für das UI-Banner
  function isWhitelisted(url, settings) {
    if (!url) return false;
    const whitelist = settings.whitelist || [];
    
    // 1. Reguläre Ausdrücke (RegEx) priorisiert prüfen
    for (const rawEntry of whitelist) {
      ...
      if (entry.startsWith('/') && entry.endsWith('/') && entry.length > 2) {
        try {
          ...
          // Toleranz-Korrektur für nicht-escapete Protokoll-Schrägstriche
          ...
          console.error(`[ZeroRAM Popup] Fehlerhafter RegEx-Eintrag: ${entry}`, e);
        }
      }
    }

    // 2. Standard-Verhalten für Domains und exakte URLs (Fallback)
    try {
      const u = new URL(url);
      return whitelist.some(entry => {
        if (!entry) return false;
        if (entry.startsWith('/') && entry.endsWith('/')) return false; // RegEx ignorieren, da oben erledigt
  ...
  // Überarbeiteter Matcher, um den exakten Eintrag für den "Entfernen"-Button zurückzugeben
  function getMatchedWhitelistEntry(url, settings) {
    if (!url) return null;
    const whitelist = settings.whitelist || [];
    
    // 1. Prüfen, ob ein regulärer Ausdruck gematcht hat
    for (const rawEntry of whitelist) {
      if (typeof rawEntry !== 'string') continue;
      const entry = rawEntry.trim();
      ...
      if (entry.startsWith('/') && entry.endsWith('/') && entry.length > 2) {
        try {
          ...
          const regex = new RegExp(pattern, 'i');
          if (regex.test(url)) {
            return entry; // Gibt den RegEx-String zurück
          }
        } catch (e) {}
      }
    }

    // 2. Standard-Matcher Fallback
    try {
      const u = new URL(url);
