...
function applySuspendedIndicator(mode) {
    ...
    if (mode === 'titlePrefix') {
      ...
      // NEU: Nutze das Custom Icon auch im TitlePrefix-Modus
      setFavicon(favicon || getFaviconURL(originalUrl));
    } else {
...
function applyTransparentFavicon(pageUrl, fallbackFaviconUrl) {
    // NEU: Verhindert den CORS-Fehler!
    // Prüfen, ob es ein lokales Base64-Bild ist (Custom Icon)
    const isBase64 = fallbackFaviconUrl && fallbackFaviconUrl.startsWith('data:');
    
    // Wenn Base64, lade dieses. Wenn es eine fremde Web-URL ist, nutze die sichere Chrome-API!
    const fetchUrl = isBase64 ? fallbackFaviconUrl : getFaviconURL(pageUrl);

    function setFallbackFavicon() {
      setFavicon(fallbackFaviconUrl || fetchUrl);
    }
