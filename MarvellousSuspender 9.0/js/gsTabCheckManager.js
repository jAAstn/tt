...
async function getUpdatedTab(tab) {
    ...
    try {
      ...
      // Fehler stumm abfangen, wenn der Tab in der Zwischenzeit geschlossen wurde
    }
...
const attemptDiscarding =
 ...
 // KORREKTUR: try...catch Block hinzugefügt, um den Verbindungsfehler abzufangen
let suspendInfo = {};
    try {
      ...
      if (isExpected) {
        gsUtils.log(tab.id, QUEUE_ID, 'Tab not ready yet', error);
      } else {
        gsUtils.warning(tab.id, QUEUE_ID, 'Unexpected error in getSuspendInfo', error);
        resolve(gsUtils.STATUS_UNKNOWN); // Tab aus Queue entfernen statt endlos warten
        return;
      }
    }
